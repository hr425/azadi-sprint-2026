...
const QUESTIONS = [
  ...
  { text: "Identify the place.", hint: "This place is in Punjab and became a powerful symbol of the freedom struggle after a tragic event in 1919.", answer: "Jallianwala Bagh" },
  ...
  { text: "Identify this place.", hint: "This historic monument in the heart of Delhi has witnessed the hoisting of the Indian national flag on Independence Day since 1947.", answer: "Red Fort" },
  { text: "Who was the first Indian woman president of the Indian National Congress?", answer: "Sarojini Naidu" },
  { text: "Who is the freedom fighter shown in these historic photographs?", hint: "His journey took him from Calcutta to Germany and later to Southeast Asia, where he played a major role in India's struggle for independence.", answer: "Subhas Chandra Bose" },
  { text: "What historic movement is captured in these photographs?", hint: "It began in Gujarat and covered roughly 390 km on foot, challenging a British law that affected something found abundantly along India's coastline.", answer: "Dandi March / Salt March" },
...
function publicState(){ return {status:session.status,phase:session.phase,questionIndex:session.questionIndex,phaseStartedAt:session.phaseStartedAt,phaseDurationMs:session.phaseDurationMs,questionCount:QUESTIONS.length,participants:Object.values(session.participants).map(p=>({id:p.id,name:p.name}))}; }
function broadcastState(){ io.emit("state",publicState()); }
function emitPhase(){ const q=session.questionIndex>=0?QUESTIONS[session.questionIndex]:null; io.emit("phase",{phase:session.phase,questionIndex:session.questionIndex,total:QUESTIONS.length,text:q?.text||"",hint:q?.hint||"",answer:q?.answer||"",startedAt:session.phaseStartedAt,durationMs:session.phaseDurationMs}); }
...
function startAnswerPhase(){
  ...
  timers.push(setTimeout(()=>{if(session.status!=="running"||session.phase!=="answer")return;startReadyPhase();},5000));
}
function startReadyPhase(){
  ...
  timers.push(setTimeout(()=>{if(session.status!=="running"||session.phase!=="ready")return;if(session.questionIndex<QUESTIONS.length-1){session.questionIndex++;startQuestion();}else finishQuiz();},10000));
}
