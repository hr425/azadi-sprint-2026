...
const HOST_OTP_EMAIL = "hr@orthoimplantsindia.com";
...
{ text: "Which freedom fighter gave the slogan “Inquilab Zindabad”?", answer: "Bhagat Singh" },
...
{ text: "In which city is the historic Cellular Jail, also known as “Kala Pani,” located?", answer: "Port Blair, Andaman and Nicobar Islands" },
...
{ text: "Who was the first Indian woman to become president of the Indian National Congress in 1925?", answer: "Sarojini Naidu" },
...
{ text: "Which British law became the immediate reason for the Jallianwala Bagh protest in 1919?", answer: "Rowlatt Act" }
];
let timers=[]; let session=createFreshSession();
const mailTransporter = process.env.SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS
  ? nodemailer.createTransport({host:process.env.SMTP_HOST,port:Number(process.env.SMTP_PORT||465),secure:String(process.env.SMTP_SECURE||"true")==="true",auth:{user:process.env.SMTP_USER,pass:process.env.SMTP_PASS}})
  : null;
...
function startCountdown(){session.status="countdown";session.phase="countdown";session.questionIndex=-1;session.phaseStartedAt=Date.now();session.phaseDurationMs=10000;broadcastState();emitPhase();timers.push(setTimeout(()=>{if(session.status!=="countdown")return;session.phase="letsStart";session.phaseStartedAt=Date.now();session.phaseDurationMs=1500;broadcastState();emitPhase();timers.push(setTimeout(()=>{if(session.status!=="countdown")return;session.status="running";session.questionIndex=0;startQuestion();},1500));},10000));}
function startQuestion(){session.phase="question";session.phaseStartedAt=Date.now();session.phaseDurationMs=20000;broadcastState();emitPhase();timers.push(setTimeout(()=>{if(session.status!=="running"||session.phase!=="question")return;startAnswerPhase();},20000));}
function startAnswerPhase(){session.phase="answer";session.phaseStartedAt=Date.now();session.phaseDurationMs=5000;broadcastState();emitPhase();timers.push(setTimeout(()=>{if(session.status!=="running"||session.phase!=="answer")return;if(session.questionIndex<QUESTIONS.length-1)startReadyPhase();else finishQuiz();},5000));}
function startReadyPhase(){session.phase="ready";session.phaseStartedAt=Date.now();session.phaseDurationMs=10000;broadcastState();emitPhase();timers.push(setTimeout(()=>{if(session.status!=="running"||session.phase!=="ready")return;session.questionIndex++;startQuestion();},10000));}
function finishQuiz(){session.status="finished";session.phase="finished";session.phaseStartedAt=null;session.phaseDurationMs=null;broadcastState();io.emit("quizFinished",{message:"Jai Hind! Thank you for playing. Winners will be announced soon."});sendResultsToHosts();}
...
function generateHostOtp(name){const otp=String(Math.floor(100000+Math.random()*900000));otpStore.set(name,{otp,expiresAt:Date.now()+5*60*1000,attempts:0});return otp;}
async function sendHostOtp(name){if(!mailTransporter)throw new Error("SMTP is not configured on Render.");const otp=generateHostOtp(name);await mailTransporter.sendMail({from:process.env.SMTP_FROM||process.env.SMTP_USER,to:HOST_OTP_EMAIL,subject:`Azadi Sprint 2026 Host Login OTP - ${name}`,text:`Your Azadi Sprint 2026 Host Login OTP is ${otp}. It is valid for 5 minutes. If you did not request this code, ignore this email.`});}
app.use(express.static(path.join(__dirname,"public")));
io.on("connection",socket=>{sendCurrentPhase(socket);
socket.on("requestHostOtp",async({name})=>{const clean=String(name||"").trim();if(!HOSTS.has(clean)){socket.emit("loginError",{message:"You are a participant, kindly log in through Participants Login."});return;}try{await sendHostOtp(clean);socket.data.pendingHost=clean;socket.emit("hostOtpSent",{message:`A 6-digit password has been sent to ${HOST_OTP_EMAIL}.`});}catch(err){console.error("Host OTP email error:",err);socket.emit("loginError",{message:"Unable to send the Host password. Please contact the administrator."});}});
socket.on("verifyHostOtp",({name,otp})=>{const clean=String(name||"").trim();const code=String(otp||"").trim();const saved=otpStore.get(clean);if(!HOSTS.has(clean)||socket.data.pendingHost!==clean){socket.emit("loginError",{message:"Please request a new Host password."});return;}if(!saved||Date.now()>saved.expiresAt){otpStore.delete(clean);socket.emit("loginError",{message:"Password expired. Please request a new one."});return;}saved.attempts++;if(saved.attempts>5){otpStore.delete(clean);socket.emit("loginError",{message:"Too many attempts. Please request a new password."});return;}if(saved.otp!==code){socket.emit("loginError",{message:"Incorrect Host password."});return;}otpStore.delete(clean);socket.data.role="host";socket.data.name=clean;socket.data.pendingHost=null;session.host=clean;socket.emit("hostLoggedIn",{name:clean});sendCurrentPhase(socket);broadcastState();if(session.status==="finished")sendResultsToHosts();});
